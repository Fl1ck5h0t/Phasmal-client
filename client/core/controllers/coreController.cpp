...
void CoreController::initAppleController()
{
    ...
    QTimer::singleShot(0, this, [this]() { PhasmalVPN::toggleScreenshots(m_appSettingsRepository->isScreenshotsEnabled()); });
#endif
}
...
void CoreController::updateTranslator(const QLocale &locale)
{
    ...
    QDirIterator it(":/translations", QStringList("PhasmalVPN_*.qm"), QDir::Files);
    while (it.hasNext()) {
    ...
    const QString translationFilePrefix = QString(":/translations/PhasmalVPN_") + lang;
    QString strFileName = QString(":/translations/PhasmalVPN_%1.qm").arg(locale.name());
    ...
    if (m_translator->load(strFileName)) {
        ...
        if (m_translator->load(QString(":/translations/PhasmalVPN_en.qm"))) {
            QCoreApplication::installTranslator(m_translator);
        }
    }
