...
echo "[PhasmalVPN] Installer package: $INSTALLER_PATH"

if [ ! -f "$INSTALLER_PATH" ]; then
    echo "[PhasmalVPN] ERROR: Installer package not found: $INSTALLER_PATH"
    exit 1
fi

PKG_PATH="$INSTALLER_PATH"
echo "[PhasmalVPN] Using PKG: $PKG_PATH"
...
echo "[PhasmalVPN] Running installer..."
OSA_CMD='do shell script "/usr/sbin/installer -pkg '"$PKG_PATH"' -target /" with administrator privileges'
osascript -e "$OSA_CMD"

STATUS=$?
if [ $STATUS -ne 0 ]; then
    echo "[PhasmalVPN] ERROR: installer exited with status $STATUS"
    exit $STATUS
fi

echo "[PhasmalVPN] Cleaning up..."
rm -f "$INSTALLER_PATH" || true
rm -rf "$EXTRACT_DIR" 2>/dev/null || true

echo "[PhasmalVPN] Installation completed successfully"
exit 0
